<div align="center">

# 👋 Julien — Dev en construction

</div>

---

Pendant des années, j'ai bossé dans le commerce et la vente. Des environnements où on apprend vite à écouter, à convaincre, et à gérer la pression sans se plaindre.

Un jour, j'ai décidé de tourner la page. Pas par défaut — par choix.

Aujourd'hui je me forme au développement web à **Metz Numeric School** (DWWM). Ma façon d'apprendre ? Je construis des projets réels, je me plante, je comprends pourquoi, je recommence. Pas de raccourcis.

Ce que j'apporte en plus d'un parcours classique : de la maturité, de la rigueur, et l'habitude de livrer même quand c'est compliqué.

- 🔥 **En ce moment :** PHP / Symfony · JavaScript · SQL
- 🚧 **Projet fil rouge :** GEST CLUB — gestion de réservations pour clubs sportifs
- 🎯 **Objectif :** Titre DWWM → DFS en alternance dès 2026
- 📍 **Base :** Sud Moselle · Remote OK

---

## 🛠️ Ma Stack Technique

<div align="center">

**Langages**

![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white)

**Frameworks & Librairies**

![Symfony](https://img.shields.io/badge/Symfony-000000?style=for-the-badge&logo=symfony&logoColor=white)

**Data & DevOps**

![MySQL](https://img.shields.io/badge/MySQL-FF6B35?style=for-the-badge&logo=mysql&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)

**Outils**

![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white)

</div>

---

## 🚀 Projet Fil Rouge — GEST CLUB

> Application web de gestion de réservations pour clubs sportifs

**GEST CLUB** est mon projet principal, développé dans le cadre de ma formation DWWM.  
Il couvre l'ensemble du cycle de développement : conception, modélisation BDD, back-end, front-end et déploiement.

| Côté | Techno |
|------|--------|
| Back-end | PHP / Symfony |
| Base de données | MySQL / Doctrine |
| Front-end | HTML · CSS · JavaScript |
| Infra | Docker |
| Versioning | Git / GitHub |

---

## 📊 Ma Progression

<div align="center">

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=julien-dev-web&show_icons=true&theme=dark&title_color=FF6B35&icon_color=FF6B35&text_color=ffffff&bg_color=0d1117&border_color=FF6B35)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=julien-dev-web&layout=compact&theme=dark&title_color=FF6B35&text_color=ffffff&bg_color=0d1117&border_color=FF6B35)

</div>

---

## 🤝 Connectons-nous

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-FF6B35?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/TON_PROFIL)
[![Email](https://img.shields.io/badge/Email-FF6B35?style=for-the-badge&logo=gmail&logoColor=white)](mailto:julien.niedziela@yahoo.fr)

</div>

---

<div align="center">

*🟠 En reconversion · En formation DWWM · À la recherche d'un stage non remunéré du 21 juillet au 18 septembre 2026*

</div>
